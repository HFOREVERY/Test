package com.fh.controller.system.dict;
...
public class DictController extends BaseController {
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Dict");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			dictService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Dict到excel");
		...
		List<PageData> varOList = dictService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("DICT_ID"));	    //1
			vpd.put("var2", varOList.get(i).getString("DICT_KEY"));	    //2
