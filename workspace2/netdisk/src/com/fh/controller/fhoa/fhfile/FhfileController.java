...
import com.fh.util.DesFileEncrypt;
...
public class FhfileController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		pd = this.getPageData();
		pd.put("KEY", MD5.md5(pd.getString("KEY")));
	...
	public ModelAndView list(Page page) throws Exception{
		...
		String username = Jurisdiction.getUsername();	//关键词检索条件
		...
		for(int i=0;i<varList.size();i++){
			...
			npd.put("is_owner", varList.get(i).getString("is_owner"));	//文件名+扩展名
	...
	public void downExcel(HttpServletRequest request, HttpServletResponse response)throws Exception{
		PageData pd = new PageData();
		pd = this.getPageData();
		String key = pd.getString("KEY1");
		String own = pd.getString("OWN");
		pd = fhfileService.findById(pd);
		String fileName = pd.getString("FILEPATH");
		DesFileEncrypt a = new DesFileEncrypt(key);
		if(own.equals("OWN")){
			a = new DesFileEncrypt();
		}
		String md5Key = MD5.md5(key);
		if(md5Key.equals(pd.getString("DEPARTMENT_ID"))){
			a = new DesFileEncrypt();
		}
		a.decrypt(PathUtil.getClasspath() + Const.FILEPATHFILEOA + fileName,PathUtil.getClasspath() + Const.FILEPATHFILEOA + "dec-" + fileName);
		FileDownload.fileDownload(response, PathUtil.getClasspath() + Const.FILEPATHFILEOA + "dec-" +fileName, pd.getString("NAME")+fileName.substring(19, fileName.length()));
		File file = new File(PathUtil.getClasspath() + Const.FILEPATHFILEOA + "dec-" +fileName);
