...
<body class="no-skin">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												...
												<input type='hidden' name='owner' value="${var.is_owner}" class="ace" />
											</td>
											...
											<td class="center" style="width:150px;">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="下载" onclick="downloadFile('${var.FHFILE_ID}','${var.is_owner}');">
													
														   <i class="ace-icon fa fa-cloud-download bigger-120" title="下载"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.FHFILE_ID}','${var.is_owner}');">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.add == 1 }">
									<a class="btn btn-mini btn-success" onclick="share('请选择分享对象');">分享</a>
									</c:if>
	...
	<script type="text/javascript">
		...
		//删除
		function del(Id,isOwner){
		    if(isOwner != "owned"){
		    	bootbox.alert("您非该文件的上传者，请联系他删除");
		    	return;
		    }
		...
		function downloadFile(id,isOwner){
			var tmpInput=$("<input type='hidden' id='FHFILE_ID' name='FHFILE_ID'/>");
	        tmpInput.attr("value", id);
	        $('#downloadForm').append(tmpInput);
			
			if(isOwner != "owned"){		    	
		    	bootbox.prompt("您非该文件的上传者，请输入秘钥", function(result){ 
		    		var tmpInput2=$("<input type='hidden' id='KEY1' name='KEY1'/>");
			        tmpInput2.attr("value", result);
			        $('#downloadForm').append(tmpInput2);
			        var tmpInput3=$("<input type='hidden' id='OWN' name='OWN'/>");
			        tmpInput3.attr("value", 'NOT_OWN');
			        $('#downloadForm').append(tmpInput3);
			        $('#downloadForm').attr("action","<%=basePath%>/fhfile/download.do" );
	    			$('#downloadForm').submit();
		    	});
		    	return;
		    }
			var tmpInput3=$("<input type='hidden' id='KEY1' name='KEY1'/>");
	        tmpInput3.attr("value", "1");
	        $('#downloadForm').append(tmpInput3);
			var tmpInput2=$("<input type='hidden' id='OWN' name='OWN'/>");
	        tmpInput2.attr("value", 'OWN');
	        $('#downloadForm').append(tmpInput2);
			$('#downloadForm').attr("action","<%=basePath%>/fhfile/download.do" );
	    	$('#downloadForm').submit();
		}
		...
		function share(msg){
					...
					if(str==''){
						...
						return;
						 ...
						 diag.Drag=true;
						 diag.Title ="新增";
						 diag.URL = "<%=basePath%>user/listCommonUsers.do?DATA_IDS="+str;
						 diag.Width = 460;
						 diag.Height = 690;
						 diag.Modal = true;				//有无遮罩窗口
						 diag.ShowMaxButton = true;	//最大化按钮
